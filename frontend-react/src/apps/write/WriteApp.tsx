...
import { CheckCircle2, Copy, FileText, Loader2, PenLine, Plus, Trash2 } from "lucide-react";
...
export function WriteApp() {
  ...
  const loadList = useCallback(async () => {
    try { setRows((await api.get<{ documents: WrittenDocRow[] }>("/api/writing")).documents); } catch { setRows([]); }
  }, []);
  ...
  async function create() {
    setBusy(true);
    try { const d = await api.post<WrittenDoc>("/api/writing", { kind: "letter", content: { add_closing: true } }); await loadList(); open(d.id); }
  ...
  const Row = ({ r }: { r: WrittenDocRow }) => (
    <button onClick={() => open(r.id)} className={cn("w-full text-left rounded-lg px-3 py-2 transition", openId === r.id ? "bg-primary/15 ring-1 ring-primary/40" : "hover:bg-muted")}>
      <div className="flex items-center gap-2 text-sm font-medium truncate">
        {r.status === "final" ? <CheckCircle2 className="w-3.5 h-3.5 shrink-0 text-emerald-500" /> : <PenLine className="w-3.5 h-3.5 shrink-0 text-muted-foreground" />}
        <span className="truncate">{r.title || "Ohne Betreff"}</span>
      </div>
      <div className="text-[11px] text-muted-foreground truncate pl-5">{r.recipient?.name || "ohne Empfänger"} · {(r.finalised_at || r.updated_at).slice(0, 10).split("-").reverse().join(".")}</div>
    </button>
  );
      ...
      <div className="flex-1 min-h-0 flex flex-col md:flex-row pb-24 md:pb-28">
        ...
        <aside className={cn("md:w-72 shrink-0 md:border-r border-border flex flex-col min-h-0", openId ? "hidden md:flex" : "flex flex-1 md:flex-none")}>
          <div className="p-4 flex items-center justify-between gap-2">
            ...
            <button onClick={create} disabled={busy} className="flex items-center gap-1.5 rounded-lg bg-primary text-primary-foreground px-3 py-1.5 text-sm font-medium disabled:opacity-50">
              {busy ? <Loader2 className="w-4 h-4 animate-spin" /> : <Plus className="w-4 h-4" />} Brief
            </button>
          </div>
          <div className="flex-1 min-h-0 overflow-y-auto px-2 pb-4 space-y-4">
            ...
            {rows && rows.length === 0 && (
              <p className="px-3 text-sm text-muted-foreground">Noch nichts geschrieben. Lege einen Brief an, oder sag es Yorik im Chat: „Schreib der Hausverwaltung, dass die Heizung kalt ist.“</p>
            )}
            ...
            {finals.length > 0 && <section><h2 className="px-3 mb-1 text-[11px] uppercase tracking-wider font-semibold text-muted-foreground">Verschickt und abgelegt</h2>{finals.map(r => <Row key={r.id} r={r} />)}</section>}
          </div>
        ...
        <main className={cn("flex-1 min-w-0 min-h-0 flex-col", openId ? "flex" : "hidden md:flex")}>
          ...
          {doc && (
            <LetterEditor key={doc.id} doc={doc} say={say} onBack={() => open(null)}
                          onChanged={d => { setDoc(d); setRows(rs => rs && rs.map(r => r.id === d.id ? { ...r, title: d.title, recipient: d.recipient, status: d.status, updated_at: d.updated_at, finalised_at: d.finalised_at } : r)); }}
