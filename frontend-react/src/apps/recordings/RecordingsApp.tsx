...
interface Task { title: string; person: string; due_date: string; why: string; task_id?: number; adopted_by?: string; adopted_at?: string }
interface Report {
...
function RecordingDetail({ id, onBack, onDeleted }: { id: number; onBack: () => void; onDeleted: () => void }) {
  ...
  async function adopt(i: number) {
    setBusy(`adopt-${i}`);
    try {
      setReport(await api.post<Report>(`/api/recordings/${id}/tasks/${i}/adopt`, {}));
          ...
          {report.tasks.length > 0 && (
            <Section title="Tasks" hint="Proposals until someone adopts them">
              {report.tasks.map((t, i) => (
                <div key={i} className="flex items-start justify-between gap-3 py-2 border-t border-border first:border-t-0">
                  <div className="min-w-0">
                    <div className="font-medium">{t.title}</div>
                    <div className="text-xs text-muted-foreground">
                      {[t.person, t.due_date].filter(Boolean).join(" · ")}{t.why ? ` — ${t.why}` : ""}
                    </div>
                  </div>
                  {t.task_id
                    ? <span className="shrink-0 flex items-center gap-1 text-xs text-emerald-600"><Check className="w-3.5 h-3.5" /> added{t.adopted_by ? ` by ${t.adopted_by}` : ""}</span>
                    : <button onClick={() => adopt(i)} disabled={busy === `adopt-${i}`}
                        ...
                        {busy === `adopt-${i}` ? <Loader2 className="w-3 h-3 animate-spin" /> : "Adopt"}
                      </button>}
                </div>
              ))}
            </Section>
