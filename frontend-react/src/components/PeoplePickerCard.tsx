  ...
  return (
    <div className="mt-2 border border-border rounded-xl bg-card/80 max-w-md overflow-hidden">
        ...
        {action.candidates.map(c => {
          ...
          return (
            ...
            >
              ...
              />
              {/* Face count badge */}
              <div className="absolute top-0.5 left-0.5 text-2xs bg-black/60 text-white px-1 py-0.5 rounded tabular-nums">
                {c.face_count}×
              </div>
              ...
              {hasExistingName && !isLabeled && (
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/85 to-transparent p-1 pt-3">
                  <button
                    ...
                    className="w-full text-2xs py-0.5 px-1.5 rounded bg-amber-500/90 text-white font-medium hover:bg-amber-500 transition truncate"
                ...
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent p-1 pt-3">
                  {action.missing_names.length === 1 ? (
                    <button
                      ...
                      disabled={!!labeling}
                      className="w-full text-2xs py-0.5 px-1.5 rounded bg-violet-500/90 text-white font-medium hover:bg-violet-500 transition truncate"
                    >
                    ...
                    <select
                      ...
                      disabled={!!labeling}
                      className="w-full text-2xs py-0.5 px-1 rounded bg-violet-500/90 text-white font-medium border-0 cursor-pointer truncate"
                    >
              ...
              {isLabeled && (
                <div className="absolute inset-x-0 bottom-0 bg-emerald-500/95 text-white text-2xs font-medium px-1 py-0.5 text-center truncate">
                  <Check className="w-2.5 h-2.5 inline mr-0.5" />{assigned}
                </div>
              )}
              {errors[c.id] && (
                <div className="absolute inset-0 bg-red-500/80 text-white text-2xs p-1 flex items-center justify-center text-center">
                  {errors[c.id]}
                </div>
              )}
            </div>
      ...
      <div className="px-3 py-2 border-t border-border/60 bg-muted/30 flex items-center gap-2">
        <span className="text-2xs text-muted-foreground flex-1">
          {labeledIds.size > 0
        ...
        <button
          onClick={() => done(true)}
          className="text-xs px-2 py-1 rounded text-muted-foreground hover:text-foreground hover:bg-muted inline-flex items-center gap-1"
        >
        ...
        <button
          ...
          className={cn(
            "text-xs px-3 py-1 rounded font-medium inline-flex items-center gap-1",
            labeledIds.size === 0
