...
def _startup() -> None:
    ...
    # Contact identity: phone channels written before September 2026 are
    # digit strings; bring them to E.164 once so WhatsApp ids, vCards and
    # signatures match. Idempotent, a few hundred rows at most.
    try:
        from .contact_identity import normalize_existing_phone_channels
        normalize_existing_phone_channels()
    except Exception as exc:  # noqa: BLE001
        logging.getLogger("yorik.contact_identity").warning("phone normalisation skipped: %s", exc)
...
def add_contact_channel_route(
    ...
    try:
        ...
        existing = _ident.owner_of(body.kind, body.value)
        owner = existing["display_name"] if existing else "another contact"
        raise HTTPException(409, detail={
            "message": f"channel already linked to {owner}",
            "existing_contact_id": int(existing["id"]) if existing else None,
        })
    return _contacts.get(contact_id) or {}
