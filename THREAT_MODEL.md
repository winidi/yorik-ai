...
| Threat | How |
|---|---|
| Cloud providers reading your data (Google, Apple, Meta) | All personal data lives in a Postgres database on your disk. No data calls to third-party APIs by default. |
| Subscription lock-in / "Sunset Day" | AGPL-3.0 + a plain Postgres database (`pg_dump` any time) means you can pick up your data and run forever. |
...
| Lost backup theft | Backups are age-encrypted (Settings → Backup; `yorik backup-verify <snapshot>` proves one restores). Encrypted blobs are safe to store on cloud drives. |
...
### 5. The LLM never writes SQL

Every read and write the model performs goes through a named skill; each `backend/skills/*/skill.md` declares which roles may call it and the registry enforces that. `backend/auth.py:ROLE_TABLES` still gates the REST endpoints per role.

### 6. Optional age encryption for backups

Backup snapshots are age-encrypted blobs. The recipient key lives in your `data/.credential_key`. Encrypted blobs are safe to push to cloud storage.
