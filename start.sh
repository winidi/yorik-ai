...
# The bridge refuses requests without this token. Generated once into
# config.env (the backend's env) and mirrored into .env (docker
# compose's env) so both sides hold the same value. Must happen before
# uvicorn starts, which is why it lives up here.
if ! grep -qE "^YORIK_WA_BRIDGE_TOKEN=." "$CONFIG_FILE"; then
  _tok="$(openssl rand -hex 24)"
  sed -i '/^YORIK_WA_BRIDGE_TOKEN=/d' "$CONFIG_FILE"
  printf '\n# Shared secret between Yorik and the bundled WhatsApp bridge (generated by start.sh).\nYORIK_WA_BRIDGE_TOKEN=%s\n' "$_tok" >> "$CONFIG_FILE"
  export YORIK_WA_BRIDGE_TOKEN="$_tok"
  echo "[init] generated YORIK_WA_BRIDGE_TOKEN in $CONFIG_FILE"
fi
touch .env
if ! grep -qE "^YORIK_WA_BRIDGE_TOKEN=" .env; then
  echo "YORIK_WA_BRIDGE_TOKEN=${YORIK_WA_BRIDGE_TOKEN}" >> .env
elif [[ "$(grep -E '^YORIK_WA_BRIDGE_TOKEN=' .env | head -1 | cut -d= -f2-)" != "${YORIK_WA_BRIDGE_TOKEN}" ]]; then
  sed -i "s|^YORIK_WA_BRIDGE_TOKEN=.*|YORIK_WA_BRIDGE_TOKEN=${YORIK_WA_BRIDGE_TOKEN}|" .env
fi
