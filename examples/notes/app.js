...
(function () {
  ...
  function renderCompose() {
    ...
    function doSave() {
      ...
      renderCompose();
      window.yorik.callOperation("notes.add_note", { body: body }).then(function (result) {
        state.saving = false;
        state.draft = "";
  ...
  function runSummary() {
    ...
    window.yorik.callOperation("notes.summarize_today").then(function (result) {
  ...
  function loadNotes() {
    return window.yorik.callOperation("notes.list_notes", { limit: 100 }).then(function (result) {
      state.loading = false;
      state.error = null;
